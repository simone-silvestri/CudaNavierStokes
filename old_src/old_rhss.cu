...
__global__ void RHSDeviceYL(myprec *rY, myprec *uY, myprec *vY, myprec *wY, myprec *eY,
	...
	for (int j = id.tiy; j < my; j += id.bdy) {
		int glb = sum + j * mx ;
		eY[glb] = eY[glb]  - 0.5 * d_workY[glb]*d_workY1[glb];
	}
...
//	fluxQuadyL(d_workY1,r,v,id);
//	for (int j = id.tiy; j < my; j += id.bdy) {
//		int glb = sum + j * mx ;
//		rY[glb] = d_workY1[glb];
//	}
//	fluxCubeyL(d_workY1,r,v,u,id);
//	for (int j = id.tiy; j < my; j += id.bdy) {
//		int glb = sum + j * mx ;
//		uY[glb] = uY[glb] + d_workY1[glb];
//	}
//	fluxCubeyL(d_workY1,r,v,v,id);
//	for (int j = id.tiy; j < my; j += id.bdy) {
//		int glb = sum + j * mx ;
//		vY[glb] = vY[glb] + d_workY1[glb];
//	}
//	fluxCubeyL(d_workY1,r,v,w,id);
//	for (int j = id.tiy; j < my; j += id.bdy) {
//		int glb = sum + j * mx ;
//		wY[glb] = wY[glb] + d_workY1[glb];
//	}
//	fluxCubeyL(d_workY1,r,v,h,id);
//	for (int j = id.tiy; j < my; j += id.bdy) {
//		int glb = sum + j * mx ;
//		eY[glb] = eY[glb] + d_workY1[glb];
//	}

	//Adding here the terms d (phi) dy * d (mu) dy; (lambda in case of h in rhse);
	...
	for (int j = id.tiy; j < my; j += id.bdy) {
		int glb = sum + j * mx ;
		vY[glb] = vY[glb] + d_workY[glb]*d_workY2[glb];
	}
	derDev1yL(d_workY,w,id); // d_work = d (w) dy
	for (int j = id.tiy; j < my; j += id.bdy) {
		int glb = sum + j * mx ;
	...
	derDev1yL(d_workY2,lam,id); //d_work2 = d (lam) dy
	derDev1yL(d_workY ,t,id);   // d_work = d (h) dy  //Change to dt!!!!
	for (int j = id.tiy; j < my; j += id.bdy) {
		int glb = sum + j * mx ;
		eY[glb] = eY[glb] + d_workY[glb]*d_workY2[glb];
	}
...
__global__ void RHSDeviceZL(myprec *rZ, myprec *uZ, myprec *vZ, myprec *wZ, myprec *eZ,
	...
	for (int k = id.tiy; k < mz; k += id.bdy) {
		int glb = k * mx * my + sum;
		eZ[glb] = eZ[glb] - 0.5*d_workZ[glb]*d_workZ1[glb];
	}
...
__global__ void RHSDeviceSharedX(myprec *rX, myprec *uX, myprec *vX, myprec *wX, myprec *eX,
	...
	derDevShared1x(&wrk1,s_t[sj],si); //wrk1 = d (h) dx
