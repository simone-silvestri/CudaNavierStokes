...
__global__ void deviceRHSY(myprec *rY, myprec *uY, myprec *vY, myprec *wY, myprec *eY,
	...
	__shared__ myprec s_u[lPencils][my/nDivY+stencilSize*2];
	__shared__ myprec s_v[lPencils][my/nDivY+stencilSize*2];
	__shared__ myprec s_w[lPencils][my/nDivY+stencilSize*2];
	__shared__ myprec s_prop[lPencils][my/nDivY+stencilSize*2];
	__shared__ myprec s_dil[lPencils][my/nDivY+stencilSize*2];
	...
	derDevSharedV1y(&wrk1,s_u[sj],si); 
	derDevSharedV1y(&wrk2,s_v[sj],si); 
	derDevSharedV1y(&wrk3,s_w[sj],si); 
	//initialize momentum RHS with stresses so that they can be added for both viscous terms and viscous heating without having to load additional terms
	uYtmp = (    wrk1 + dvdx[id.g]        );
	vYtmp = (2 * wrk2 - 2./3.*s_dil[sj][si] );
	wYtmp = (    wrk3 + dvdz[id.g]        );

	//adding the viscous dissipation part duidy*mu*siy
	eYtmp = s_prop[sj][si]*(uYtmp*wrk1 + vYtmp*wrk2 + wYtmp*wrk3);
